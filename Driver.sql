/*
/ Metric Database that allows for the use of users to be
/ inputted such as PM,Estimators, or implementation for future 
/ users.
/
/ Tyler Fung
/
...
--App user table that creates the fields for the use of users. First user_if will be an auto incremented integer 
--starting from one. 
...
--P.O.P. is calculated based on its perfomance in days. Project is assigned to a manager with
...
--table that allos for the allocation of CLIN numbers which is the same as projects.
--assigns CLN to specific numbers
CREATE TABLE clin (
...
--CLIN assigned to various orgnizations through metric database management. 
CREATE TABLE org (
...
--must be instantiated to assign different packages to different organizations. 
...
--Assign various tasks to work packages, id, task name, start date, end date, 
CREATE TABLE task (
